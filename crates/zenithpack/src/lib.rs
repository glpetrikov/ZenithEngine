...
// const MAGIC: &[u8; 14] = b"ZENITHPACK0.1\0";
...
// 	InvalidPath(String),
// 	FileNotFound(String),
// 	MemoryLimit { needed: u64, available: u64 },
...
// 			Self::InvalidArchive(message) => write!(f, "invalid zenithpack archive:
// {message}"), 			Self::InvalidPath(path) => write!(f, "invalid asset path:
// {path}"), 			Self::FileNotFound(path) => write!(f, "file not found in
// zenithpack archive: {path}"), 			Self::MemoryLimit { needed, available } =>
// write!( 				f,
// 				"not enough memory to load archive: needed {needed} bytes, {available}
// bytes available" 			),
// 			Self::Ruzstd(error) => write!(f, "failed to decode zenithpack compressed
// stream: {error}"), 			Self::Utf8(error) => write!(f, "{error}"),
// 		}
// 	}
// }
...
// 	fn from(error: ruzstd::decoding::errors::FrameDecoderError) -> Self {
// Self::Ruzstd(error) } }
...
// #[derive(Debug)]
// pub struct ZenithpackArchive {
// 	path: PathBuf,
// 	index: ZenithpackIndex,
// 	data_offset: u64,
// }

// #[derive(Debug)]
// pub struct SmartZenithpack {
// 	archive: ZenithpackArchive,
// 	mode: SmartMode,
// }
...
// impl ZenithpackIndex {
// 	pub fn contains(&self, path: &str) -> bool { self.files.contains_key(path) }

// 	pub const fn total_uncompressed_size(&self) -> u64 {
// self.total_uncompressed_size }

// 	pub fn files(&self) -> impl Iterator<Item = &str> {
// self.files.keys().map(String::as_str) } }

// impl ZenithpackArchive {
// 	/// opens a Zenithpack archive from the given file path and reads its index
// 	/// into memory.
// 	///
// 	/// # Errors
// 	/// This function will return an error if the input file is not a valid
// 	/// Zenithpack archive, or if there is an I/O error while reading the file.
...
// 		let index = ZenithpackIndex::decode(&index_bytes)?;
...
// 	pub const fn index(&self) -> &ZenithpackIndex { &self.index }
...
// impl SmartZenithpack {
// 	pub fn open(path: impl AsRef<Path>) -> Result<Self> {
// 		let archive = ZenithpackArchive::open(path)?;

// 		let mut sys = sysinfo::System::new();
// 		sys.refresh_memory();
// 		let available = sys.available_memory();
// 		let needed = archive.index.total_uncompressed_size();

// 		let mode = if needed <= available / 3 {
...
// 		} else {
// 			SmartMode::Disk { root: TempDir::new()? }
// 		};
...
// 		normalize_archive_path(path).is_ok_and(|path|
// self.archive.index.contains(&path)) 	}
...
// 	pub fn materialize_to_temp(&self) -> Result<TempDir> {
...
// pub fn pack_directory(input_dir: impl AsRef<Path>, output_file: impl
// AsRef<Path>) -> Result<()> { 	pack_directory_filtered(input_dir, output_file,
// |_| true) }
...
// 		!path
// 			.components()
// 			.any(|component| matches!(component, Component::Normal(name) if
// excluded_names.iter().any(|excluded| name == OsStr::new(excluded)))) 	})
// }

// /// # Errors
// /// This function will return an error if the input file is not a valid
// /// Zenithpack archive, or if there is an I/O error while reading or writing
// /// files.
// pub fn unpack_file(input_file: impl AsRef<Path>, output_dir: impl
// AsRef<Path>) -> Result<()> { 	let archive =
// ZenithpackArchive::open(input_file)?; 	fs::create_dir_all(output_dir.
// as_ref())?;
...
// Error::InvalidArchive("file is too large"))?; 		let tar_offset =
// 			u64::try_from(tar_bytes.len()).map_err(|_| Error::InvalidArchive("tar
// stream is too large"))? + 512;
...
// index.total_uncompressed_size.saturating_add(size); 	}
...
// 	ruzstd::encoding::compress_to_vec(input,
...
// 			.strip_prefix(root)
// 			.map_err(|_| Error::InvalidArchive("failed to build relative asset
// path"))?;
...
// 	let padding = (TAR_BLOCK_LEN as u64 - size % TAR_BLOCK_LEN as u64) %
// TAR_BLOCK_LEN as u64; 	output.resize(output.len() +
// usize::try_from(padding).unwrap_or(0), 0); }

// fn copy_exact(mut reader: impl Read, mut writer: impl Write, len: u64) ->
// Result<()> { 	let mut limited = reader.by_ref().take(len);
...
// 	usize::try_from(value).map_err(|_| Error::InvalidArchive("value does not fit
// in memory")) }
...
// 			let mut path = vec![0; usize::try_from(path_len).map_err(|_|
// Error::InvalidArchive("path is too long"))?]; 			input.read_exact(&mut
// path)?; 			let path = normalize_archive_path(&String::from_utf8(path)?)?;
