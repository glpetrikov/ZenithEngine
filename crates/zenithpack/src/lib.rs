...
impl fmt::Display for Error {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::InvalidArchive(message) => write!(f, "invalid zenithpack archive: {message}"),
			Self::InvalidPath(path) => write!(f, "invalid asset path: {path}"),
			Self::FileNotFound(path) => write!(f, "file not found in zenithpack archive: {path}"),
			Self::MemoryLimit { needed, available } => write!(
				f,
				"not enough memory to load archive: needed {needed} bytes, {available} bytes available"
			),
			Self::Ruzstd(error) => write!(f, "failed to decode zenithpack compressed stream: {error}"),
...
pub fn unpack_file(input_file: impl AsRef<Path>, output_dir: impl AsRef<Path>) -> Result<()> {
	let archive = ZenithpackArchive::open(input_file)?;
