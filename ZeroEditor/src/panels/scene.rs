...
use crate::{editor_workspace::EditorMode, undo_redo::SceneSnapshotCommand};
...
impl Panel for ScenePanel {
	...
	fn show(&mut self, ui: &mut Ui, context: &mut EditorPanelContext<'_>) {
		egui::Frame::NONE.inner_margin(0.0).outer_margin(0.0).show(ui, |ui| {
			...
			if self.pending_scene_edit.is_some() && !ui.input(|input| input.pointer.button_down(PointerButton::Primary))
			{
...
impl ScenePanel {
	...
	fn apply_2d_camera_controls(&mut self, ui: &Ui, viewport: egui::Rect, context: &mut EditorPanelContext<'_>) {
		...
		let pan_active = self.rmb_camera_active && !ui.input(|input| input.pointer.button_down(PointerButton::Primary));
