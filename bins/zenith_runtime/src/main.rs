...
fn main() -> zenith_error::ZResult<()> {
...

	{
		...
		let _ = world.add_component(entity, Transform::default());
...
fn try_something() -> zenith_error::ZResult<()> {
	std::fs::read_to_string("this_file_does_not_exist.txt")?;
	Ok(())
}
